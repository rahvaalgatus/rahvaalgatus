...
describe("LOCAL_GOVERNMENTS", function() {
	...
	it("must have a non-zero voter count on all governments", function() {
		_.each(LOCAL_GOVERNMENTS, function(gov) {
			gov.voterCount.must.be.at.least(1)
			gov.voterCount.must.be.at.most(1000000)
		})
	})
	...
	it("must have a signature threshold on all governments", function() {
		_.each(LOCAL_GOVERNMENTS, function(gov) {
			var threshold = Math.max(Math.round(gov.voterCount * 0.01), 5)
