...
describe("HomeController", function() {
	...
	function mustShowInitiativesInPhases(path, dest) {
		describe("as a filtered home page", function() {
	...
	describe("GET /", function() {
		...
		describe("recent initiatives", function() {
			it("must show initiatives with recent signatures", function*() {
				...
				recents.childNodes.forEach(function(li, i) {
					var el = li.firstChild
					var initiative = initiatives[i]
					el.href.must.equal(Initiative.slugPath(initiative))
					el.querySelector("h3").textContent.must.equal(initiative.title)

					li.querySelector(".note").textContent.must.equal(
						t("RECENTLY_SIGNED")
					)
				})
			})
			...
			it("must order based on last update", function*() {
				...
				recents.childNodes.forEach(function({firstChild: el}, i) {
					var initiative = initiatives[i]
					el.href.must.equal(Initiative.slugPath(initiative))
					el.querySelector("h3").textContent.must.equal(initiative.title)
				})
	...
	describe("GET /parliament", function() {
		mustShowInitiativesInPhases("/parliament", "parliament")
	...
	describe("GET /local", function() {
		mustShowInitiativesInPhases("/local", "muhu-vald")

		it(`must not show statistics`, function*() {
			var res = yield this.request("/local")
			res.statusCode.must.equal(200)
			var dom = parseHtml(res.body)
