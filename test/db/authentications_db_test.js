...
describe("AuthenticationsDb", function() {
	...
	describe(".create", function() {
		it("must permit duplicate country and personal ids", function() {
			authenticationsDb.create(new ValidAuthentication({
				country: "EE",
		...
		it("must throw given duplicate tokens", function() {
			...
			var err
			try {
				authenticationsDb.create(new ValidAuthentication({token: auth.token}))
			}
			catch (ex) { err = ex }
		...
		it("must throw given longer country", function() {
			var err
			try {
				authenticationsDb.create(new ValidAuthentication({country: "EST"}))
			}
			catch (ex) { err = ex }
			err.must.be.an.error(SqliteError)
			err.code.must.equal("constraint")
		...
		it("must throw given lowercase country", function() {
			var err
			try { authenticationsDb.create(new ValidAuthentication({country: "ee"})) }
			catch (ex) { err = ex }
			err.must.be.an.error(SqliteError)
			err.code.must.equal("constraint")
			err.type.must.equal("check")
