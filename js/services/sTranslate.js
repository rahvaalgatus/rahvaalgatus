...
app.service("sTranslate", [ "$translate", "$filter", "$log", function($translate, $filter, $log) {
    ...
    var _fieldErrorsToKeys = function(errorResponse, model) {
        ...
        Object.keys(errors).forEach(function(key) {
            var translationKey = FIELD_ERROR_KEY_PATTERN.replace(":statusCode", errorResponse.data.status.code).replace(":model", model.toUpperCase()).replace(":fieldName", key.toUpperCase());
    ...
    var _generalErrorToKey = function(errorResponse, model) {
        ...
        var translationKey = GENERAL_ERROR_KEY_PATTERN.replace(":statusCode", errorResponse.data.status.code).replace(":model", model.toUpperCase());
        var translationKeyFallback = GENERAL_ERROR_FALLBACK_KEY_PATTERN.replace(":statusCode", errorResponse.data.status.code);
        // The key exists
        if (translationKey !== Translate.translate(translationKey)) {
