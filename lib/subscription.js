...
exports.serializeMailgunVariables = serializeMailgunVariables
...
exports.renderEventsEmail = function(t, initiative, events) {
	...
	if (events.length == 1) {
		...
		switch (event.type) {
			case "text": return {
				...
				text: renderEmail("EMAIL_INITIATIVE_TEXT_EVENT_MESSAGE_BODY", {
					...
					initiativeTitle: initiative.title,
					initiativeUrl: initiativeSlugUrl
				})
			}
		...
		text: renderEmail("INITIATIVE_PARLIAMENT_EVENT_MESSAGE_BODY", {
			initiativeTitle: initiative.title,
			initiativeUrl: initiativeSlugUrl,
			eventsUrl: `${initiativeSlugUrl}#events`,
...
exports.send = function*(msg, subscriptions) {
	for (
		...
		if (msg.id) messagesDb.update(msg, {updated_at: new Date, sent_to: sent})
	}

	if (msg.id) messagesDb.update(msg, {updated_at: new Date, sent_at: new Date})
}
...
function sendBatch(msg, subscriptions) {
	...
	var recipients = serializeMailgunVariables(subscriptions)
...
function serializeMailgunVariables(subscriptions) {
	return _.fromEntries(subscriptions.map((sub) => [sub.email, {
