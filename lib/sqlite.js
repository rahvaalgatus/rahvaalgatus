...
Statement.prototype.all = function(params) {
	var fn = this.statement.all.bind(
		this.statement,
		params && params.map(serialize) || []
	)
...
Statement.prototype.get = function(params) {
	var fn = this.statement.get.bind(
		this.statement,
		params && params.map(serialize) || []
	)
...
Statement.prototype.run = function(params) {
	var fn = this.statement.run.bind(
		this.statement,
		params && params.map(serialize) || []
	)
...
Statement.prototype.iterate = function(params) {
	try { return this.statement.iterate(params && params.map(serialize) || []) }
	catch (e) { throw e instanceof Sqlite3.SqliteError ? new SqliteError(e) : e }
}
...
function transactSync(sqlite, fn) {
	...
	try { var res = fn(); sqlite(sql`RELEASE transacting`); return res }
	catch (err) {
		sqlite(sql`ROLLBACK TO transacting`)
		sqlite(sql`RELEASE transacting`)
		throw err
	}
}
