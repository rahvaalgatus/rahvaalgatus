...
var {parseDate} = require("root/lib/time")
...
exports.isDeadlineOk = function(from, deadline) {
	from = DateFns.startOfDay(from)
	var min = DateFns.addDays(from, MIN_DEADLINE_DAYS)
	var max = DateFns.addDays(from, MAX_DEADLINE_DAYS)
...
exports.getExpirationDate = function(initiative) {
	var startedOn = _.max([
		...
		parseDate(Config.expireSignaturesFrom)
	])
...
function initiativeUrl(initiative) {
	var host = isLocalInitiative(initiative) ? Config.localUrl : Config.url
