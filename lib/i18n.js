...
exports.markdown = function(lang, key) {
	if (lang == "xx") return Path.join("locales", "*", key + ".md")
	var html = MARKDOWNS[lang][key]
	if (html != null) return html

	var path = Path.join(LOCALES, lang, key + ".md")
	return MARKDOWNS[lang][key] = readMarkdown(Fs.existsSync(path)
		? path
		: Path.join(LOCALES, DEFAULT_LANGUAGE, key + ".md")
	)
}
...
exports.formatDate = function(format, date) {
	switch (format) {
		...
		case "iso-week": return formatDateTime(date, "GGGG-[W]WW")
...
exports.formatBytes = function(bytes) {
	...
	return roundTo(3, bytes / Math.pow(1024, exp)) + BYTE_SIZES[exp]
}

exports.nameParliamentCommittee = function(lang, id) {
	return id in PARLIAMENT_COMMITTEES
		? t(lang, "shared.parliament_committees." + id.replace(/-/g, "_"))
		: id
}
