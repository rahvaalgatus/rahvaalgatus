...
exports.canPropose = function(now, initiative) {
	...
	return (
		...
		exports.can("admin", initiative) &&
		(now >= min || hasTag(initiative, "fast-track"))
	)
}
...
exports.canSendToParliament = function(initiative, dbInitiative) {
	return (
		...
		exports.can("admin", initiative) && (
			exports.isSuccessful(initiative, dbInitiative) ||
			initiative.vote &&
			exports.countSignatures("Yes", initiative) >= 1 &&
			dbInitiative.has_paper_signatures
		)
	)
}
...
function hasTag(initiative, tag) {
	return initiative.categories && initiative.categories.includes(tag)
}
