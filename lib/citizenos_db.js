...
exports.setTitlesFromTopics = function*(initiatives) {
	...
	initiatives.forEach(function(initiative) {
		...
		var title = parseTitle(topic.description)
		initiative.title = title || topic.title
	})
}
...
function parseTitle(html) {
	var title = html.match(TITLE_REGEXP)
	return title ? decodeEntities(title[2]).trim() : null
}
